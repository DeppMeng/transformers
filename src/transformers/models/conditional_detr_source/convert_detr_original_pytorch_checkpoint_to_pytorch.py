...
from conditional_detr_original.models import build_model
...
@torch.no_grad()
def convert_detr_checkpoint(model_name, original_model_path, pytorch_dump_folder_path, args):
    """
    ...
    # detr = torch.load(original_model_path)
    conditional_detr, _, _ = build_model(args)
    
    checkpoint = torch.load(args.original_model_path, map_location='cpu')
    conditional_detr.load_state_dict(checkpoint['model'])
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--original_model_path", default=None, type=str, help="Path to the original PyTorch model path."
    )
    parser.add_argument(
    ...
    # Conditional DETR model parameters
    ...
    convert_detr_checkpoint(args.model_name, args.original_model_path, args.pytorch_dump_folder_path, args)
